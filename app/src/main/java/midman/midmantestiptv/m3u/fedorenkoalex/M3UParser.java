...
public class M3UParser {
    ...
    public M3UPlaylist parseFile(InputStream inputStream) throws FileNotFoundException {
        ...
        List<Channel> playlistItems = new ArrayList<Channel>();
        ...
        for (int i = 0; i < linesArray.length; i++) {
            ...
            if (currLine.contains(EXT_M3U)) {
                ...
                //Log.i("ERRORE_INDEX", String.valueOf(i) + " : " + dataArray[0] + "  --  " + dataArray[1]);
                ...
                String name = "";
                try {
                    name = dataArray[1].substring(0, dataArray[1].indexOf(EXT_URL)).replace("\n", "").replace("\r", "");
                } catch (Exception e){
                    name = "Channel Name not found! (" + String.valueOf(i) + ")";
                }

                String url="";
                try {
                    url = dataArray[1].substring(dataArray[1].indexOf(EXT_URL)).replace("\n", "").replace("\r", "");
                } catch (Exception e){
                    continue;
                }
